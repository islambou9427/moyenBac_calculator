...
public class moy_gest extends AppCompatActivity {
    Button btn_back,btn_result;
    EditText note_sc,note_phis,note_math,note_arab,note_fr,note_eng,note_fal,note_geo,note_isl,note_sport,note_fa;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.moy_gest);

        btn_back = findViewById(R.id.btn_back);
        note_fa = findViewById(R.id.note_fa);
        ...
        btn_back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent a = new Intent(moy_gest.this, filiere.class);
                startActivity(a);
            }
        });
    ...
    public void shawMessage() {
        ...
        if(note_sc.length()==0 && note_arab.length()==0 && note_phis.length()==0 && note_math.length()==0 && note_eng.length()==0 && note_fal.length()==0 && note_fr.length()==0 && note_geo.length()==0 && note_isl.length()==0 && note_sport.length()==0){
            ...
            note_fa.setError("Entrer la note");
        }
        else {
            ...
            double n12 = Double.parseDouble(note_fa.getText().toString());
            double n11 = (n1 * 6 + n2 * 5 + n3 * 5 + n4 * 4 + n5 * 3 + n6 * 2 + n7 * 2 + n8 * 2 + n9 * 2 + n10 + n12 *2 ) / 34 ;
